...
def verify_scale(rotate_rect):############################################3
    error = 0.4
    aspect = 4  # 4.7272
...
def img_transform(car_rect,img):###################################################################4
    img_h,img_w = img.shape[:2]
    rect_w,rect_h,angle= car_rect[1][0],car_rect[1][1],car_rect[2]
    ...
    if flag:
        car_img = img[int(car_rect[0][1] - rect_h / 2):int(car_rect[0][1] + rect_h / 2),int(car_rect[0][0] - rect_w / 2):int(car_rect[0][0] + rect_w / 2)]
        return car_img
    ...
    if left_point[1] <= right_point[1]:  # 正角度
        ...
        dst = cv2.warpAffine(img, M, (round(img_w * 2), round(img_h * 2)))
...
def pre_process(orig_img):#原始图像预处理######################################################################5
    gray_img = cv2.cvtColor(orig_img,cv2.COLOR_BGR2GRAY)#1.转灰度图
    #cv2.imshow('gray_img', gray_img)

    blur_img = cv2.GaussianBlur(gray_img, (3, 3),0.5)#2.均值模糊，柔化小的噪点
    #cv2.imshow('blur', blur_img)

    sobel_img = cv2.Sobel(blur_img,cv2.CV_16S,1,0,ksize=3)#对x求偏导，对y不求偏导
    sobel_img = cv2.convertScaleAbs(sobel_img)#3.sobel获取垂直边缘，转为uint8,否则无法显示图像
    cv2.imshow('sobel', sobel_img)

    hsv_img = cv2.cvtColor(orig_img, cv2.COLOR_BGR2HSV)
    #h色调0-180，s饱和度0-255，v明度0-255
    h,s,v = hsv_img[:,:,0],hsv_img[:,:,1],hsv_img[:,:,2]
    # 黄色色调区间[26，34],蓝色色调区间:[100,124]
    blue_img = (((h > 26) & (h < 34)) | ((h > 100) & (h < 124))) & (s > 70) & (v > 70)#numpy按条件截取
    ...
    kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (21, 5))#21*5矩形结构元素
    close_img = cv2.morphologyEx(binary_img, cv2.MORPH_CLOSE, kernel)#形态学滤波
    cv2.imshow('pred_img', close_img)
    cv2.waitKey(0)
    return close_img

# 给候选车牌区域做漫水填充算法，一方面补全上一步求轮廓可能存在轮廓歪曲的问题，
# 另一方面也可以将非车牌区排除掉
def verify_color(rotate_rect,src_img):#######################################
    ...
    box_points_x = [point[0] for point in box_points]
    box_points_x.sort(reverse=False)
    adjust_x = int((box_points_x[2] - box_points_x[1]) * adjust_param)
    col_range = [box_points_x[1]+adjust_x,box_points_x[2]-adjust_x]####3
    box_points_y = [point[1] for point in box_points]
    ...
    if (col_range[1] - col_range[0]) / (box_points_x[3] - box_points_x[0]) < 0.4 or (row_range[1] - row_range[0]) / (box_points_y[3] - box_points_y[0]) < 0.4:
        points_row = []
        points_col = []
        for i in range(2):#分别再2条对角线上生成种子点
    ...
    for i in range(rand_seed_num):
        ...
        if (((h[row, col] > 26) & (h[row, col] < 34)) | ((h[row, col] > 100) & (h[row, col] < 124))) & (
            ...
            cv2.floodFill(src_img, mask, (col, row), (255, 255, 255), (loDiff,) * 3, (upDiff,) * 3, flags)#白色填充，偏移值30
            cv2.circle(flood_img, center=(col, row), radius=2, color=(0, 0, 255), thickness=2)#填充种子点,红色
            seed_cnt += 1
...
def locate_carPlate(orig_img,pred_img):###############################################
    ...
    contours,heriachy = cv2.findContours(pred_img,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)#只检测外轮廓且只保留该方向的终点坐标，例如一个矩形轮廓只需4个点来保存轮廓信息

    print("总共有%d个轮廓",len(contours))
    for i,contour in enumerate(contours):
        cv2.drawContours(temp1_orig_img,contours,i,(0, 255, 255), 2)#h黄色轮廓
        ...
        if verify_scale(rotate_rect):
            ...
            car_plate = cv2.resize(car_plate, (136, 36))  # 调整尺寸为后面CNN车牌识别做准备
            # ========================调试看效果========================#
            ...
            # ========================调试看效果========================#
            carPlatelist.append(car_plate)

    cv2.imshow('contour', temp1_orig_img)
    cv2.waitKey(0)
    print (len(carPlatelist))
    return carPlatelist
...
def horizontal_cut_chars(plate):
    ...
    for i in range(img_w):
        ...
        if colValue > col_limit:
            if is_char_flag == False:#进入非字符区域,计算上一个字符区域
                area_right = round((i + char_right) / 2)
                area_width = area_right - area_left
                char_width = char_right - char_left#计算上一个字符区域长度
...
def get_chars(carplate):
    img_h,img_w = carplate.shape[:2]
    h_proj_list = []  # 水平投影长度列表
    h_temp_len,v_temp_len = 0,0
    h_startIndex,h_end_index = 0,0 # 水平投影记索引
    h_proj_limit = [0.15,0.8] # 车牌在水平方向得轮廓长度少于20%或多余80%过滤掉
    char_imgs = []
    ...
    for row in range(img_h):
        temp_cnt = 0
        for col in range(img_w):
            if carplate[row,col] == 255:
                temp_cnt += 1
        h_count[row] = temp_cnt
        if temp_cnt / img_w < h_proj_limit[0] or temp_cnt / img_w > h_proj_limit[1]:#碰到非字符区域
            if h_temp_len != 0:
                h_end_index = row - 1
    ...
    plates = carplate[chars_top:chars_bottom+1,:]
    cv2.imshow("car",carplate)
    cv2.imshow("plate",plates)
    cv2.waitKey(0)
    char_addr_list = horizontal_cut_chars(plates)##############函数调用

    for i,addr in enumerate(char_addr_list):
        char_img = carplate[chars_top:chars_bottom + 1, addr[0]:addr[1]]
        char_img = cv2.resize(char_img, (20,20))
        char_imgs.append(char_img)
        cv2.imshow('charimg',char_img)
        cv2.waitKey(0)
    return char_imgs

def extract_char(carplate):
    gray_plate = cv2.cvtColor(carplate,cv2.COLOR_BGR2GRAY)
    ret,binary_plate = cv2.threshold(gray_plate,0,255,cv2.THRESH_BINARY|cv2.THRESH_OTSU)
    #cv2.imshow('binary_p',binary_plate)
...
if __name__ == '__main__':
    img = cv2.imread('E:/MyDownloads/images/pictures/42.jpg')
    plate_model_path = 'E:\MyDownloads\images\model\plate\model.ckpt-530.meta'
    char_model_path = 'E:\MyDownloads\images\model\char\model.ckpt-590.meta'
    cv2.imshow('image', img)
    cv2.waitKey(0)
    pred_img = pre_process(img)

    carplate_list = locate_carPlate(img,pred_img)

    ret,carplate = cnn_select_carPlate(carplate_list,plate_model_path)
    if ret == False:
        print("未检测到车牌")
        sys.exit(-1)
    #cv2.imshow('plate',carplate)

    char_imgs = extract_char(carplate)

    text = cnn_recongnize_char(char_imgs,char_model_path)
    print (text)
