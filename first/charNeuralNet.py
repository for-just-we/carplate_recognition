...
class char_cnn_net():
    ...
    def cnn_construct(self):
        ...
        conv1 = tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(x_input,cw1,strides=[1,1,1,1],padding='SAME'),cb1))
        ...
        fw3 = tf.Variable(tf.random_normal(shape=[1024, len(self.dataset)], stddev=0.01), dtype=tf.float32)
        fb3 = tf.Variable(tf.random_normal(shape=[len(self.dataset)]), dtype=tf.float32)#最终len个输出
    ...
    def train(self,data_dir,model_path):
        ...
        predicts = tf.argmax(predicts, axis=1)#原来predict每个元素为一个概率列表，argmax后predict为最有可能的值的索引
        actual_y = tf.argmax(self.y_place, axis=1)#actual_y为真实的值的索引
        ...
        with tf.Session() as sess:
            ...
            while True:
                ...
                if step % 10 == 0:
                    ...
                    print(len(test_index),len(test_randx))
    ...
    def test(self,x_images,model_path):
        ...
        with tf.Session() as sess:
            ...
            for i in range(len(preds)):
                ...
                test_list.append(self.dataset[pred])
            return test_list


if __name__ == '__main__':
    data_dir = 'E:\MyDownloads\images\cnn_char_train'
    test_dir = 'E:\MyDownloads\images\cnn_char_test'
    train_model_path = 'E:\MyDownloads\images\model\char\model.ckpt'
    model_path = 'E:\MyDownloads\images\model\char\model.ckpt-590'
