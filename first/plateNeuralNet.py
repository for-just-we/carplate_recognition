...
class plate_cnn_net:
    ...
    def cnn_construct(self):
        x_input = tf.reshape(self.x_place, shape=[-1, self.img_h, self.img_w, 3])#输入136*36图像，通道数3 ,[训练时一个batch的图片数量, 图片高度, 图片宽度, 图像通道数]

        cw1 = tf.Variable(tf.random_normal(shape=[3, 3, 3, 32], stddev=0.01), dtype=tf.float32)#卷积核为3*3大小，3通道，输出32个特征
        cb1 = tf.Variable(tf.random_normal(shape=[32]), dtype=tf.float32)#
        conv1 = tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(x_input, filter=cw1, strides=[1, 1, 1, 1], padding='SAME'), cb1))#卷积层+线性整流层,卷积层输出图像分辨率不变
        conv1 = tf.nn.max_pool(conv1, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')#池化层
        conv1 = tf.nn.dropout(conv1, self.keep_place)

        cw2 = tf.Variable(tf.random_normal(shape=[3, 3, 32, 64], stddev=0.01), dtype=tf.float32)#3*3大小卷积核，32通道，64输出特征
        ...
        cw3 = tf.Variable(tf.random_normal(shape=[3, 3, 64, 128], stddev=0.01), dtype=tf.float32)#3*3卷积核
        ...
        fw1 = tf.Variable(tf.random_normal(shape=[17 * 5 * 128, 1024], stddev=0.01), dtype=tf.float32)#1024输出
        ...
        fully3 = tf.add(tf.matmul(fully2, fw3), fb3, name='out_put')#最终2输出
    ...
    def train(self,data_dir,model_save_path):
        ...
        out_put = self.cnn_construct()#创建神经网络
        predicts = tf.nn.softmax(out_put)#求出属于某一类的概率
        predicts = tf.argmax(predicts, axis=1)#取出最大概率的类
        actual_y = tf.argmax(self.y_place, axis=1)#二分类实际值
        accuracy = tf.reduce_mean(tf.cast(tf.equal(predicts, actual_y), dtype=tf.float32))#求准确率
        cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=out_put, labels=self.y_place))#二次代价函数，求代价值，即需要优化的值
        opt = tf.train.AdamOptimizer(self.learn_rate)
        train_step = opt.minimize(cost)#升级版梯度下降算法
        ...
        with tf.Session() as sess:
            ...
            while True:
                ...
                _, loss = sess.run([train_step, cost], feed_dict={self.x_place: train_randx,
                                                                  self.y_place: train_randy, self.keep_place: 0.75})#每轮计算代价值并优化
                step += 1
                print(step, loss)
                ...
                if step % 10 == 0:
                    ...
                    acc = sess.run(accuracy, feed_dict={self.x_place: test_randx,
                                                        self.y_place: test_randy, self.keep_place: 1.0})#每10轮计算准确率
...
if __name__ == '__main__':
    data_dir = 'E:\MyDownloads\images\cnn_plate_train'
    test_dir = 'E:\MyDownloads\images\cnn_plate_test'
    train_model_path = 'E:\MyDownloads\images\model\plate\model.ckpt'
    model_path = 'E:\MyDownloads\images\model\plate\model.ckpt-530'
